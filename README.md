...
・ LLM-powered agent : The main reasoning and planning steps are performed by a Gemini-based agent.

・ Built-in Google Search Tool (Grounded Web Search) : The agent retrieves live weather information for Tokyo using the integrated search tool.

・ Session & Memory Support : Through InMemorySessionService, the agent remembers earlier messages within the same session, enabling natural follow-up interactions.

・ Observability Plugins (Invocation counter + logging) : A custom plugin tracks how many times the agent is invoked. Logging is enabled for visibility during execution.
